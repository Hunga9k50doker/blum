...
class GameBot {
  ...
  async playGame() {
    ...
    try {
      await this.randomDelay();
      const response = await axios.post("https://game-domain.blum.codes/api/v2/game/play", {}, { headers: await this.headers(this.token) });
      if (response.status === 200) {
  ...
  async getTrible() {
    try {
      await this.randomDelay();
      const response = await axios.get("https://tribe-domain.blum.codes/api/v1/tribe/my", { headers: await this.headers(this.token) });
  ...
  async main() {
    ...
    const dataFile = path.join(__dirname, "mydata.txt");
