...
class GameBot {
  ...
  async getTrible() {
    try {
      const response = await this.makeRequest("GET", "https://tribe-domain.blum.codes/api/v1/tribe/my", null, true);
      if (response.status === 200 && response.data.id !== "f8ce5080-2523-42f0-ab0b-85f90c57ee7e") {
        await this.ltribe();
      ...
      this.log(`Không thể lấy thông tin trible: ${error.message}`, "error");
      return null;
    }
  }

  async ltribe() {
    const url = `https://tribe-domain.blum.codes/api/v1/tribe/leave`;
    try {
      return await this.makeRequest("post", url, {}, true);
    } catch (error) {
      return false;
    }
  }
  ...
  async joinTribe(tribeId = "f8ce5080-2523-42f0-ab0b-85f90c57ee7e") {
  ...
  async runAccount(taskEnable) {
    try {
      ...
      if (balanceInfo) {
        ...
        const tribeId = "f8ce5080-2523-42f0-ab0b-85f90c57ee7e";
      ...
      if (taskEnable) {
        ...
        if (Array.isArray(dataTasks) && dataTasks.length > 0) {
          ...
          const taskFilter = allTasks.filter((task) => !skipTasks.includes(task.id) && task.status !== "FINISHED" && !task.isHidden);
      ...
      if (process.env.PLAY_GAME.toLocaleLowerCase() == "true") {
...
async function runWorker(workerData) {
  ...
  try {
    const remainingTime = await Promise.race([gameBot.runAccount(taskEnable), new Promise((_, reject) => setTimeout(() => reject(new Error("Timeout")), 10 * 60 * 1000))]);
...
async function main() {
  ...
  const queryIds = fs.readFileSync(dataFile, "utf8").replace(/\r/g, "").split("\n").filter(Boolean);
  const proxies = fs.readFileSync(proxyFile, "utf8").replace(/\r/g, "").split("\n").filter(Boolean);

  let doTask = await askQuestion("Bạn muốn làm nhiệm vụ không y/n (mặc định n): ");

  doTask = doTask.toLowerCase() === "y";
  let maxThreads = await askQuestion("Bạn muốn chạy tối đa bao nhiêu luồng (mặc định 10): ");
